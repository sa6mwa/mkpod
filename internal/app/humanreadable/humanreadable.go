package humanreadable

import "fmt"

// SI() and IEC() are Copyright (C) Stefan Nilsson
// Source: Format byte size as kilobytes, megabytes, gigabytes, ...
// https://yourbasic.org/golang/formatting-byte-size-to-human-readable-format/
// SI and IEC were originally called ByteCountSI and ByteCountIEC

/*
	Convert byte size to bytes, kilobytes, megabytes, gigabytes, ...

	These utility functions convert a byte size to a human-readable string
	in either SI (decimal) or IEC (binary) format.

	Source: https://yourbasic.org/golang/formatting-byte-size-to-human-readable-format/
*/

func SI(b int64) string {
	const unit = 1000
	if b < unit {
		return fmt.Sprintf("%d B", b)
	}
	div, exp := int64(unit), 0
	for n := b / unit; n >= unit; n /= unit {
		div *= unit
		exp++
	}
	return fmt.Sprintf("%.1f %cB",
		float64(b)/float64(div), "kMGTPE"[exp])
}

func IEC(b int64) string {
	const unit = 1024
	if b < unit {
		return fmt.Sprintf("%d B", b)
	}
	div, exp := int64(unit), 0
	for n := b / unit; n >= unit; n /= unit {
		div *= unit
		exp++
	}
	return fmt.Sprintf("%.1f %ciB",
		float64(b)/float64(div), "KMGTPE"[exp])
}
