...
func TestLoadAllowsExplicitEpisodeImagesWithoutDefaultPodImage(t *testing.T) {
	specFile := writeTempSpec(t, `
atom: "podcast.rss"
title: "Test Podcast"
...
- uid: 1
  ...
  input: "masters/episode.flac"
`)

	atom, err := New(specFile).Load(context.Background())
	if err != nil {
		t.Fatalf("Load() error = %v, want explicit episode images without defaultPodImage to load", err)
	}
	if atom.Config.DefaultPodImage != "" {
		t.Fatalf("DefaultPodImage = %q, want empty", atom.Config.DefaultPodImage)
	}
}
...
func writeTempSpec(t *testing.T, content string) string {
	...
	if _, err := tmpFile.WriteString(strings.TrimSpace(content)); err != nil {
		tmpFile.Close()
