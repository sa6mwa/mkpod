...
# End-to-end test for remote master removal safety checks
# Tests the --remove-remote-master flag safety behavior
...
# Cleanup function - always restore backups
cleanup() {
    local exit_code=$?
    ...
    # Only preserve non-zero exit codes (errors)
...
# Setup: Upload master files to input bucket
...
# Test 1: Normal case - local file exists and is adequate size
...
# Test 2: Safety check - no local master file  
...
if $MKPOD_BIN encode 2 --remove-remote-master --force; then
    # Check if the remote file still exists (it should)
...
# Test 3: Safety check - local file too small
