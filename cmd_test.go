...
func TestPlanEpisodeShowsEncodingDecision(t *testing.T) {
	...
	writeTestWAV(t, filepath.Join(target, "masters", "episode.wav"), 44100, 1)
	...
	updated := strings.Replace(string(specContent), "preferredFormat: mp3", "preferredFormat: m4a", 1)
	updated = strings.Replace(updated, "episodes: []", `episodes:
    - uid: 1
      title: "Episode One"
      pubDate: "Fri, 25 Mar 2022 16:00:13 +0000"
      link: "https://example.com/podcast/episodes/1"
      subtitle: "Integration test episode"
      description: "Generated by TestPlanEpisodeShowsEncodingDecision"
      input: "masters/episode.wav"
`, 1)
	if err := os.WriteFile(specPath, []byte(updated), 0o644); err != nil {
		t.Fatalf("write updated spec: %v", err)
	...
	output, err := cmdTest("", "plan", "episode", "--spec", specPath, "1")
	if err != nil {
		t.Fatalf("mkpod plan episode failed: %v\nOutput: %s", err, output)
	}
	for _, want := range []string{
		"Workflow: episode",
		"Episode: 1",
		"Encode mode: ffmpeg-audio",
		"Preferred format: m4a",
		"Output: episode.m4a",
		"Will encode: true",
		"Encode reason: local output is missing",
...
func TestPlanEpisodeShowsExistingOutputWouldPrompt(t *testing.T) {
	target := filepath.Join(t.TempDir(), "podcast")
	if _, err := cmdTest("", "init", target); err != nil {
		t.Fatalf("mkpod init failed: %v", err)
	}
	writeTestWAV(t, filepath.Join(target, "masters", "episode.wav"), 44100, 1)
	if err := os.WriteFile(filepath.Join(target, "episode.mp3"), []byte("placeholder"), 0o644); err != nil {
    ...
    - uid: 1
      ...
      link: "https://example.com/podcast/episodes/1"
      subtitle: "Integration test episode"
      description: "Generated by TestPlanEpisodeShowsExistingOutputWouldPrompt"
      input: "masters/episode.wav"
`, 1)
	if err := os.WriteFile(specPath, []byte(updated), 0o644); err != nil {
		t.Fatalf("write updated spec: %v", err)
	...
	for _, want := range []string{
		"Output exists: true",
		"Will encode: false",
		"Encode reason: local output exists; apply episode would prompt before re-encoding",
	} {
		if !strings.Contains(output, want) {
			t.Fatalf("expected output to contain %q, got: %s", want, output)
		}
