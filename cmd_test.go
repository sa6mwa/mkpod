...
func TestCLICommands(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: "Generate the local podcast RSS feed",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cmd := exec.Command("go", append([]string{"run", "-buildvcs=true", "."}, tt.args...)...)
...
func TestPlanCommandRemoved(t *testing.T) {
	output, err := cmdTest("", "plan", "--help")
	if err == nil {
		t.Fatalf("mkpod plan unexpectedly succeeded\nOutput: %s", output)
	}
	if !strings.Contains(output, `unknown command "plan"`) {
		t.Fatalf("expected unknown plan command, got: %s", output)
	}
}
...
func cmdTest(_ string, args ...string) (string, error) {
	command := exec.Command("go", append([]string{"run", "-buildvcs=true", "."}, args...)...)
...
func compiledBinary(t *testing.T) string {
	...
	compiledBinaryOnce.Do(func() {
		...
		cmd := exec.Command("go", "build", "-buildvcs=true", "-o", compiledBinaryPath, ".")
		output, err := cmd.CombinedOutput()
		if err != nil {
