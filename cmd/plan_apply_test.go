...
func TestPersistRepairOutputRejectsMissingProductionKey(t *testing.T) {
	specFile := writeWorkflowSpecFixture(t)

	err := persistRepairOutput(context.Background(), specFile, 1, workflow.Operation{
		Kind:       workflow.OperationRepairMetadata,
		ObjectKind: workflow.ObjectProductionAudio,
	})
	if err == nil {
		t.Fatal("persistRepairOutput() error = nil, want missing key error")
	}
	if !strings.Contains(err.Error(), "requires production output key") {
		t.Fatalf("persistRepairOutput() error = %v, want missing key", err)
	}
}
