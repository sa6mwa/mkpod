...
func applyRenewEpisodePlan(ctx context.Context, plan *newEpisodePlan) error {
	...
	if !renewEpisodePlanMatchesExisting(&episode, &atom.Episodes[index]) {
		return fmt.Errorf("episode UID %d already exists with different metadata", episode.UID)
	}
	return nil
}
...
func renewEpisodePlanMatchesExisting(planned, existing *model.Episode) bool {
	if !newEpisodePlanMatchesExisting(planned, existing) {
		return false
	}
	if planned == nil || existing == nil {
		return false
	}
	return strings.TrimSpace(planned.Output) == strings.TrimSpace(existing.Output) &&
		strings.TrimSpace(planned.Type) == strings.TrimSpace(existing.Type) &&
		planned.Length == existing.Length &&
		planned.Duration.Duration == existing.Duration.Duration &&
		planned.PubDate.Time.Equal(existing.PubDate.Time)
}
