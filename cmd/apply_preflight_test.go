...
func TestBuildNewEpisodeApplyDecisionRemoteOnlyMasterFallsBackFromGenericContentType(t *testing.T) {
	specFile := writeWorkflowSpecFixture(t)
	plan := &newEpisodePlan{
		SpecFile: specFile,
		Episode:  episodeFixtureForNewPlan(2),
	}
	plan.Episode.Input = "masters/new.mp4"
	remote := &applyPreflightRemote{infos: map[string]*s3store.FileInfo{
		"input/masters/new.mp4": {Exists: true, Size: 100, ContentType: "application/octet-stream"},
	}}
	...
	requireWorkflowOperation(t, decision, workflow.OperationEncode)
	upload := requireWorkflowOperation(t, decision, workflow.OperationUploadProduction)
	if upload.Bucket != "output" || upload.Key != "new.mp4" {
		t.Fatalf("upload operation = %+v, want output/new.mp4", upload)
	}
}
